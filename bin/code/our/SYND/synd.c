...
extern byte F1[N][R8A] __attribute((aligned(16)));
extern byte F2[N][R8A] __attribute((aligned(16)));

inline void apply(byte F[N][R8A], byte in[R8A], byte out[R8A])
{
	asm
	(
		    "xorps %xmm0, %xmm0"
...
int main(int argc, char** argv)
{
	...
	byte y    [R8A] __attribute__((aligned(16)));
	byte z    [R8A] __attribute__((aligned(16)));
	byte state[R8A] __attribute__((aligned(16)));
	...
	while (!feof(inFile))
	{
		...
		apply(F1, state, state); // update the state
		apply(F2, state, NULL);  // the outpu of the keystreamt is only set in SSE registers
		asm
		(
			    "xorps     (%0), %%xmm0"
			"\n\tmovaps %%xmm0,    (%0)"
