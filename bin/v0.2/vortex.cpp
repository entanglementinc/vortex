...
void generatePublicKey(const unsigned int& k, const unsigned int& n, unsigned char **S, unsigned char **G, unsigned char **P, unsigned char **K, unsigned char **key, const unsigned int& t, unsigned char *z);
...
void msgDecrypt(const unsigned int& n, unsigned char **c, unsigned char **PInverse, unsigned char **KInverse, const unsigned int& k, unsigned char **SInverse, unsigned char **msg, unsigned char **GRM, const unsigned int& r, const unsigned int& m);
...
int main()
{
	...
	unsigned char** K = (unsigned char**)malloc(sizeof(unsigned char*) * n);
	unsigned char** KInverse = (unsigned char**)malloc(sizeof(unsigned char*) * n);
	for (int i = 0; i < n; i++)
	{		
		P[i] = (unsigned char*)malloc(sizeof(unsigned char) * n);
		PInverse[i] = (unsigned char*)malloc(sizeof(unsigned char) * n);
		K[i] = (unsigned char*)malloc(sizeof(unsigned char) * n);
		KInverse[i] = (unsigned char*)malloc(sizeof(unsigned char) * n);
	}

	//printf("P ");
	permutationMatrix(K, KInverse, n); //generate P
	inverseMatrix(K, KInverse, n); 
	permutationMatrix(K, KInverse, n); //generate P
	inverseMatrix(K, KInverse, n); //calcurate P inverse	
	/*printf("P\n");
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
	...
	generatePublicKey(k, n, S, G, P, K, publicKeyG, publicKeyt, z[0]); //generate public key G' and error pattern z
	/*printf("public key G'\n");
	for (int i = 0; i < k; i++)
	{
		for (int j = 0; j < n; j++)
...
for (int msgLoop = 0; msgLoop < 1; msgLoop++)
{
	...
	for (int tLoop = publicKeyt; tLoop <= publicKeyt; tLoop++)
	{
		...
		msgDecrypt(n, c, PInverse, KInverse, k, SInverse, decipher, G, r, m); //decrypt recived meassage
...
void generatePublicKey(const unsigned int& k, const unsigned int& n, unsigned char** S, unsigned char** G, unsigned char** P, unsigned char **K, unsigned char** key, const unsigned int& t, unsigned char* z)
{
	...
	unsigned char **buffer = (unsigned char**)malloc(sizeof(unsigned char*) * k);

	for (int i = 0; i < k; i++)
	{
		buffer[i] = (unsigned char*)malloc(sizeof(unsigned char) * n);
	}

	mtrixMultiplication(k, k, S, k, n, G, pub);
	
	mtrixMultiplication(k, n, pub, n, n, P, buffer);
	mtrixMultiplication(k, n, buffer, n, n, P, key);
...
void msgDecrypt(const unsigned int& n, unsigned char** c, unsigned char** PInverse, unsigned char **KInverse, const unsigned int& k, unsigned char** SInverse, unsigned char** msg, unsigned char** GRM, const unsigned int& r, const unsigned int& m)
{
	...
	buffer[0] = (unsigned char*)malloc(sizeof(unsigned char) * n);
	mtrixMultiplication(1, n, c, n, n, (unsigned char**)KInverse, (unsigned char**)buffer);
	mtrixMultiplication(1, n, buffer, n, n, PInverse,(unsigned char**) cPrime);
	/*printf("c prime\n");
	for (int i = 0; i < n; i++)
	{
	printf("%d ", cPrime[0][i]);
